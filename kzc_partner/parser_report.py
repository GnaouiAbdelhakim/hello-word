...
class PartnerReport(report_sxw.rml_parse):

    # fuction init

    def __init__(self, cr, uid, name, context):
        super(PartnerReport, self).__init__(cr, uid, name, context=context)
    ...
    def _lines_get(self, partner):
        ...
        invoices = invoice_obj.search(self.cr, self.uid,[('partner_id','=',partner.id),('type','in',('out_invoice','out_refund'))],limit=10, order= 'date_invoice desc')
...
class ReportPartnerReport(osv.AbstractModel):
    _name = 'report.kzc_partner.partner_report'
    _inherit = 'report.abstract_report'
    _template = 'kzc_partner.partner_report'

    _wrapped_report_class = PartnerReport
